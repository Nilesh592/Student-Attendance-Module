...
class Student{
	...
	public function insertStudent($name, $city, $branch, $division, $roll){
		...
		if (empty($name) || empty($city) || empty($branch) || empty($division) || empty($roll)) {
			$msg = "<div class='alert alert-danger'><strong>Error !</strong> Field must not be empty !</div>";
			return $msg;
		} else {
			$stu_query = "INSERT INTO tbl_student(name, city, branch, division, roll) VALUES('$name', '$city', '$branch', '$division', '$roll')";
	...
	public function insertAttendance($attend = array()){
		...
		while ($result = $getdata->fetch_assoc()) {
			...
			$cur_date = date('Y-m-d');
			if ($cur_date == $db_date) {
				$msg = "<div class='alert alert-danger'><strong>Error !</strong> Attendance Already Taken Today ! Plz Go to the REPORT section and update it</div>";
				return $msg;
			}
		}
